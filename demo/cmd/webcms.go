...
type Rule struct {
	...
	Expression string            `yaml:"expression"`	 //描述要在响应报文中匹配的内容 用于验证漏洞利用成功与否
}


type POC struct {                    	//poc名称规则和详细信息
...
func init() {
	rootCmd.AddCommand(webcmsCmd)
	webcmsCmd.Flags().StringVarP(&Hosts, "hosts", "H", "", "设置你要扫描 Redis 数据库的主机")
}