...
var webthinkphpCmd = &cobra.Command{
	Use:   "webthinkphp",
	Short: "cms-poc Attack",
	Run: func(cmd *cobra.Command, args []string) {
		fmt.Printf("\033[1;31;40m%s\033[0m\n", "正在进行对url进行thinkphp-poc攻击")
		flag.Parse()
		start := time.Now()
		defer func() {
				...
				for _, rule := range poc.Rules {                 //如过yaml中有多种rules则用这个
					...
					for _, line := range abc {
						...
						body, _ := ioutil.ReadAll(resp.Body)
							...
							//检查响应是否包含预期的表达式
							if strings.Contains(string(body), rule.Expression)&&resp.StatusCode == 200 {
...
func init() {
	rootCmd.AddCommand(webthinkphpCmd)
	webthinkphpCmd.Flags().StringVarP(&Hosts, "hosts", "H", "", "设置你要扫描 Redis 数据库的主机")
}
